...
class lander{
   public:
	  ...
	  float getXPos();
	  float getXVel();
	  float getMinSafeXPos();
	  float getMaxSafeXPos();
	  float getSafeSpeed();
	  float getWind();
	  float getFuel();
	  float getAccel();
	  void targetControl(float, float, float, float, float, float, float, float *, float *);
   private:
      float height;
      float xPosition;
      float Yvelocity;
...
void lander::control(float calcBurn, float calcThrust){
...
void lander::targetControl(float y_pos, float y_vel, float x_pos, float x_vel, float wind, float Fuel, float acceleration, float *targetBurn, float *targetThrust){
	*targetBurn = acceleration + (y_vel - 4 - acceleration);
	*targetThrust = x_vel + wind;
}

float lander::lrand(void){
	srand(rand() % 10000000);
...
void lander::update(float calcBurn, float calcThrust){
    // update the lander's altitude 
    Yvelocity+=ACCELERATION;  // apply acceleration
    control(calcBurn, calcThrust);  // calculate burn and thrust
	//burn is vertical
...
float lander::getYVel(){
	return Yvelocity;
}

float lander::getXPos(){
	return xPosition;
}

float lander::getXVel(){
	return Xvelocity;
}
...
float lander::getWind(){
	return WIND;
}

float lander::getAccel(){
	return ACCELERATION;
}

float lander::getMaxSafeXPos(){
	return MAX_SAFE_X;
}

float lander::getMinSafeXPos(){
	return MIN_SAFE_X;
}

float lander::getSafeSpeed(){
	return MAX_SAFE_LANDING_SPEED;
}
